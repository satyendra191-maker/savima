-- ============================================
-- SAVIMAN COMPLETE SCHEMA WITH RLS
...
CREATE TABLE IF NOT EXISTS admin_users (
    ...
    name VARCHAR(100),
    permissions JSONB DEFAULT '{"products": true, "inquiries": true, "industries": true, "blogs": true, "settings": true, "users": true, "export": true, "leads": true, "logistics": true}',
...
VALUES ('satyendra191@gmail.com', 'super_admin', 'Satyendra', '{"products": true, "inquiries": true, "industries": true, "blogs": true, "settings": true, "users": true, "export": true, "leads": true, "logistics": true, "create": true, "delete": true, "update": true, "read": true}')
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM admin_users WHERE email = user_email AND is_active = true) THEN
        RETURN true;
    END IF;
    RETURN false;
END;
...
BEGIN
    SELECT auth.jwt()->>'email' INTO user_email;
    IF user_email IS NULL THEN RETURN false; END IF;
    SELECT permissions INTO user_permissions FROM admin_users WHERE email = user_email AND is_active = true;
...
BEGIN
    ...
    RETURN QUERY SELECT au.id, au.email, au.role, au.name FROM admin_users au WHERE au.email = user_email AND au.is_active = true;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- ADMIN USERS
CREATE POLICY "Admins can view all admins" ON admin_users FOR SELECT USING (is_admin());
CREATE POLICY "Super admins can manage admins" ON admin_users FOR ALL USING (is_admin() AND (SELECT role FROM admin_users WHERE email = (auth.jwt()->>'email') AND is_active = true) = 'super_admin');
...
CREATE POLICY "Admins can manage industries" ON industries FOR ALL USING (is_admin());
...
CREATE POLICY "Admins can delete inquiries" ON inquiries FOR DELETE USING (is_admin());
...
CREATE POLICY "Admins can view ai leads" ON ai_generated_leads FOR SELECT USING (is_admin());
CREATE POLICY "Admins can update ai leads" ON ai_generated_leads FOR UPDATE USING (is_admin());
CREATE POLICY "Admins can delete ai leads" ON ai_generated_leads FOR DELETE USING (is_admin());

-- LOGISTICS QUOTES
CREATE POLICY "Anyone can create logistics quotes" ON logistics_quotes FOR INSERT WITH CHECK (true);
CREATE POLICY "Admins can view logistics quotes" ON logistics_quotes FOR SELECT USING (is_admin());
CREATE POLICY "Admins can update logistics quotes" ON logistics_quotes FOR UPDATE USING (is_admin());
CREATE POLICY "Admins can delete logistics quotes" ON logistics_quotes FOR DELETE USING (is_admin());

-- LOGISTICS PARTNERS
CREATE POLICY "Public can view active partners" ON logistics_partners FOR SELECT USING (status = 'active');
CREATE POLICY "Admins can manage partners" ON logistics_partners FOR ALL USING (is_admin());
...
CREATE POLICY "Admins can manage catalogs" ON catalogs FOR ALL USING (is_admin());

-- CAREERS
CREATE POLICY "Public can view active careers" ON careers FOR SELECT USING (status = 'active');
CREATE POLICY "Anyone can apply for jobs" ON careers FOR INSERT WITH CHECK (true);
CREATE POLICY "Admins can manage careers" ON careers FOR ALL USING (is_admin());

-- CAREER APPLICATIONS
CREATE POLICY "Public can submit applications" ON career_applications FOR INSERT WITH CHECK (true);
CREATE POLICY "Admins can manage applications" ON career_applications FOR ALL USING (is_admin());
...
CREATE POLICY "Admins can manage blogs" ON blog_posts FOR ALL USING (is_admin());
...
CREATE TRIGGER update_ai_generated_leads_updated_at BEFORE UPDATE ON ai_generated_leads FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_logistics_quotes_updated_at ON logistics_quotes;
CREATE TRIGGER update_logistics_quotes_updated_at BEFORE UPDATE ON logistics_quotes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
VALUES 
    ...
    ('catalogs', 'catalogs', true, 52428800, ARRAY['application/pdf']),
    ...
    ('documents', 'documents', true, 20971520, ARRAY['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'image/jpeg', 'image/png'])
ON CONFLICT (id) DO NOTHING;

-- Storage Policies
CREATE POLICY "Public can view images" ON storage.objects FOR SELECT USING (bucket_id IN ('products', 'blog-images', 'site-assets', 'catalogs', 'resumes', 'documents'));
CREATE POLICY "Admins can upload" ON storage.objects FOR INSERT WITH CHECK (bucket_id IN ('products', 'blog-images', 'site-assets', 'catalogs', 'resumes', 'documents') AND is_admin());
CREATE POLICY "Admins can delete" ON storage.objects FOR DELETE USING (bucket_id IN ('products', 'blog-images', 'site-assets', 'catalogs', 'resumes', 'documents') AND is_admin());
CREATE POLICY "Admins can update" ON storage.objects FOR UPDATE USING (bucket_id IN ('products', 'blog-images', 'site-assets', 'catalogs', 'resumes', 'documents') AND is_admin());

-- ============================================
-- SEED DATA - International Logistics Partners
-- ============================================
