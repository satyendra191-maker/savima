...
import { Shield, Lock, Mail, Eye, EyeOff, Loader2, Zap, Copy, CheckCircle } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const DEMO_CREDS = [
    { label: 'Super Admin', email: 'admin@saviman.com', password: 'saviman2024', color: 'from-amber-500 to-yellow-600' },
    { label: 'Demo User', email: 'demo@saviman.com', password: 'demo1234', color: 'from-blue-500 to-cyan-600' },
  ];
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-6 sm:px-12 lg:px-16 bg-white dark:bg-gray-950">
        <div className="w-full max-w-md mx-auto">
          ...
          {/* ── DEMO CREDENTIALS ─────────────────────────────── */}
          <div className="mb-8 p-5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-2xl">
            <div className="flex items-center gap-2 mb-4">
              <Zap size={16} className="text-amber-600 dark:text-amber-400" />
              <span className="text-sm font-black text-amber-800 dark:text-amber-400 uppercase tracking-wider">
                Quick Demo Access
              </span>
            </div>
            <div className="space-y-3">
              {DEMO_CREDS.map((cred) => (
                <button
                  ...
                  className="w-full flex items-center justify-between bg-white dark:bg-gray-900 border border-amber-200 dark:border-amber-800/30 rounded-xl px-4 py-3 hover:border-amber-400 transition-all group"
                >
                  <div className="text-left">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-black bg-gradient-to-r ${cred.color} bg-clip-text text-transparent uppercase tracking-wider`}>
                        {cred.label}
                      </span>
                    </div>
                    <div className="text-xs font-mono text-gray-600 dark:text-gray-400 mt-0.5">{cred.email}</div>
                    <div className="text-xs font-mono text-gray-400 dark:text-gray-600">{cred.password}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {copied === cred.email ? (
                      <CheckCircle size={16} className="text-green-500" />
                    ) : (
                      <Copy size={16} className="text-gray-400 group-hover:text-amber-500 transition-colors" />
                    )}
                    <span className="text-xs text-amber-600 dark:text-amber-400 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                      USE →
                    </span>
                  </div>
                </button>
          ...
          <button
            ...
            className="w-full flex justify-center items-center gap-3 py-3.5 px-4 border border-gray-200 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-900 text-sm font-bold text-gray-800 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 focus:ring-2 focus:ring-brass-500 disabled:opacity-60 transition-all mb-6"
          >
            {googleLoading ? (
